...
func TestFileEdit_Replace(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Replace: &ReplaceOp{OldString: "foo", NewString: "baz"}}}}
...
func TestFileEdit_DryRun(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{f}, DryRun: true, Operations: []Op{{Replace: &ReplaceOp{OldString: "foo", NewString: "bar"}}}}
	result := fileEditCall(t, params, tmp)
...
func TestFileEdit_Glob(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{filepath.Join(tmp, "*.go")}, Operations: []Op{{Replace: &ReplaceOp{OldString: "foo", NewString: "bar"}}}}
...
func TestFileEdit_NotFound(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Replace: &ReplaceOp{OldString: "missing", NewString: "x"}}}}
	result := fileEditCall(t, params, tmp)
...
func TestFileEdit_EmptyOps(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{}}
	result := fileEditCall(t, params, tmp)
...
func TestFileEdit_MultipleInserts_UseOriginalLineNumbers(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Insert: &InsertOp{Line: 2, Content: "A\n"}}, {Insert: &InsertOp{Line: 4, Content: "B\n"}}}}
	fileEditCall(t, params, tmp)
	require.Equal(t, "1\nA\n2\n3\nB\n4\n", readFileContent(t, f))
}
...
func TestFileEdit_ConflictingReplaceAndRemove_Fails(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Replace: &ReplaceOp{OldString: "beta", NewString: "BETA"}}, {Remove: &RemoveOp{OldString: "beta gamma"}}}}
	result := fileEditCall(t, params, tmp)
...
func TestFileEdit_InsertInsideRemovedRange_Fails(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Remove: &RemoveOp{Lines: []int{2, 3}}}, {Insert: &InsertOp{Line: 3, Content: "inserted\n"}}}}
	result := fileEditCall(t, params, tmp)
	require.Contains(t, result, "conflicts")
...
func TestFileEdit_InsertAtReplaceEndBoundary_Allowed(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Replace: &ReplaceOp{OldString: "two", NewString: "TWO"}}, {Insert: &InsertOp{Line: 3, Content: "A\n"}}}}
	fileEditCall(t, params, tmp)
	require.Equal(t, "one\nTWO\nA\nthree\n", readFileContent(t, f))
}
...
func TestFileEdit_Appends_PreserveOrder(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Append: &AppendOp{Content: "A"}}, {Append: &AppendOp{Content: "B"}}}}
	fileEditCall(t, params, tmp)
...
func TestFileEdit_ReplaceIfMissing_OrdersWithAppendAtEOF(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Replace: &ReplaceOp{OldString: "missing", NewString: "x", IfMissing: "A"}}, {Append: &AppendOp{Content: "B"}}}}
	fileEditCall(t, params, tmp)
...
func TestFileEdit_InsertAutoIndent_Preserved(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Insert: &InsertOp{Line: 2, Content: "inserted\n", Indent: "auto"}}}}
	fileEditCall(t, params, tmp)
...
func TestFileEdit_InsertNone_PreservesExactContent(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Insert: &InsertOp{Line: 2, Content: "inserted", Indent: "none"}}}}
	fileEditCall(t, params, tmp)
...
func TestFileEdit_Remove_ByOldString(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Remove: &RemoveOp{OldString: "XXX "}}}}
	result := fileEditCall(t, params, tmp)
	require.NotEmpty(t, result)
...
func TestFileEdit_Remove_ByLines(t *testing.T) {
	...
	f := filepath.Join(tmp, "test.go")
	os.WriteFile(f, []byte("line1\nline2\nline3\n"), 0644)
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Remove: &RemoveOp{Lines: []int{2}}}}}
	fileEditCall(t, params, tmp)
	require.Equal(t, "line1\nline3\n", readFileContent(t, f))
}
...
func TestFileEdit_Remove_InvalidLinesLength(t *testing.T) {
	...
	f := filepath.Join(tmp, "test.go")
	os.WriteFile(f, []byte("line1\nline2\n"), 0644)
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Remove: &RemoveOp{Lines: []int{1, 2, 3}}}}}
	result := fileEditCall(t, params, tmp)
...
func TestFileEdit_Append(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Append: &AppendOp{Content: "bar"}}}}
	fileEditCall(t, params, tmp)
	require.Equal(t, "foo\nbar", readFileContent(t, f))
}
...
func TestFileEdit_Patch_AddLine(t *testing.T) {
	...
	f := filepath.Join(tmp, "test.go")
	os.WriteFile(f, []byte("line1\n"), 0644)
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Patch: &PatchOp{Patch: "@@ -1 +1,2 @@\n line1\n+line2\n"}}}}
	fileEditCall(t, params, tmp)
	require.Equal(t, "line1\nline2\n", readFileContent(t, f))
}
...
func TestFileEdit_Patch_RemoveLine(t *testing.T) {
	...
	f := filepath.Join(tmp, "test.go")
	os.WriteFile(f, []byte("line1\nline2\nline3\n"), 0644)
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Patch: &PatchOp{Patch: "@@ -1,3 +1,2 @@\n line1\n-line2\n line3\n"}}}}
	fileEditCall(t, params, tmp)
	require.Equal(t, "line1\nline3\n", readFileContent(t, f))
}
...
func TestFileEdit_Patch_ReplaceLine(t *testing.T) {
	...
	f := filepath.Join(tmp, "test.go")
	os.WriteFile(f, []byte("line1\nline2\n"), 0644)
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Patch: &PatchOp{Patch: "@@ -1,2 +1,2 @@\n line1\n-line2\n+line2-modified\n"}}}}
	fileEditCall(t, params, tmp)
	require.Equal(t, "line1\nline2-modified\n", readFileContent(t, f))
}
...
func TestFileEdit_Patch_InvalidPatch_ReturnsError(t *testing.T) {
	...
	f := filepath.Join(tmp, "test.go")
	os.WriteFile(f, []byte("foo\n"), 0644)
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Patch: &PatchOp{Patch: ""}}}}
	result := fileEditCall(t, params, tmp)
...
func TestFileEdit_PatchAndReplace_ContextLine_NonOverlappingMerge(t *testing.T) {
	...
	f := filepath.Join(tmp, "test.go")
	os.WriteFile(f, []byte("line1\nline2\nline3\n"), 0644)
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Patch: &PatchOp{Patch: "@@ -1,3 +1,3 @@\n line1\n-line2\n+line2-modified\n line3\n"}}, {Replace: &ReplaceOp{OldString: "line1", NewString: "LINE1"}}}}
	fileEditCall(t, params, tmp)
	require.Equal(t, "LINE1\nline2-modified\nline3\n", readFileContent(t, f))
}
...
func TestFileEdit_DiffInDisplayBlocks(t *testing.T) {
	...
	f := filepath.Join(tmp, "test.go")
	os.WriteFile(f, []byte("foo\n"), 0644)
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Replace: &ReplaceOp{OldString: "foo", NewString: "bar"}}}}
	tl := fileEdit()
...
func TestFileEdit_NoChangeNoDisplayBlock(t *testing.T) {
	...
	f := filepath.Join(tmp, "test.go")
	os.WriteFile(f, []byte("foo\n"), 0644)
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Replace: &ReplaceOp{OldString: "foo", NewString: "foo"}}}}
	tl := fileEdit()
	raw, err := json.Marshal(params)
	require.NoError(t, err)
	res, err := tl.Execute(ctx(tmp), raw)
...
func TestFileEdit_AllowPartial_Success(t *testing.T) {
	...
	f := filepath.Join(tmp, "test.go")
	os.WriteFile(f, []byte("foo bar\n"), 0644)
	params := FileEditParams{Path: tool.StringSliceParam{f}, AllowPartial: true, Operations: []Op{{Replace: &ReplaceOp{OldString: "foo", NewString: "baz"}}}}
	fileEditCall(t, params, tmp)
	require.Equal(t, "baz bar\n", readFileContent(t, f))
}
...
func TestFileEdit_AllowPartial_ReportsDirectorySkip(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{okFile, skipDir}, AllowPartial: true, Operations: []Op{{Replace: &ReplaceOp{OldString: "foo", NewString: "bar"}}}}
	result := fileEditCall(t, params, tmp)
...
func TestFileEdit_DefaultMode_NoPartialWriteOnSecondFileFailure(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{a, b}, Operations: []Op{{Replace: &ReplaceOp{OldString: "foo", NewString: "bar"}}}}
...
func TestFileEdit_AllowPartial_OneFails(t *testing.T) {
	...
	f := filepath.Join(tmp, "test.go")
	os.WriteFile(f, []byte("foo\n"), 0644)
	params := FileEditParams{Path: tool.StringSliceParam{f}, AllowPartial: true, Operations: []Op{{Replace: &ReplaceOp{OldString: "missing", NewString: "x"}}}}
	result := fileEditCall(t, params, tmp)
	require.Contains(t, result, "failed")
	require.Equal(t, "foo\n", readFileContent(t, f))
}
...
func TestFileEdit_LargeFile_ReturnsError(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Replace: &ReplaceOp{OldString: "a", NewString: "b", ReplaceAll: true}}}}
	result := fileEditCall(t, params, tmp)
...
func TestFileEdit_AllowPartial_ReportsLargeFileSkip(t *testing.T) {
	...
	params := FileEditParams{Path: tool.StringSliceParam{okFile, largeFile}, AllowPartial: true, Operations: []Op{{Replace: &ReplaceOp{OldString: "foo", NewString: "bar"}}}}
	result := fileEditCall(t, params, tmp)
	require.Contains(t, result, "failed")
...
func TestFileEdit_SequentialEditsWithoutConcurrentChange(t *testing.T) {
	...
	f := filepath.Join(tmp, "test.go")
	os.WriteFile(f, []byte("foo\n"), 0644)
	params1 := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Replace: &ReplaceOp{OldString: "foo", NewString: "bar"}}}}
	fileEditCall(t, params1, tmp)
	require.Equal(t, "bar\n", readFileContent(t, f))
	params2 := FileEditParams{Path: tool.StringSliceParam{f}, Operations: []Op{{Replace: &ReplaceOp{OldString: "bar", NewString: "baz"}}}}
