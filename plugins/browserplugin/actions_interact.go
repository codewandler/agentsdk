...
func (p *Plugin) executeClick(_ action.Ctx, input ClickInput) (ClickOutput, error) {
	...
	var afterURL string
	_ = chromedp.Run(ctx, chromedp.Location(&afterURL))
...
func (p *Plugin) executeType(_ action.Ctx, input TypeInput) (TypeOutput, error) {
	...
	if input.Clear {
		tasks = append(tasks, chromedp.Clear(input.Selector))
	}
	if input.Text != "" {
		tasks = append(tasks, chromedp.SendKeys(input.Selector, input.Text))
	}
	if input.Submit {
		tasks = append(tasks, chromedp.SendKeys(input.Selector, kb.Enter))
	}
...
func (p *Plugin) executeWait(_ action.Ctx, input WaitInput) (WaitOutput, error) {
	...
	// Use the smaller of: explicit timeout_ms or the configured OpTimeout.
	timeout := p.sessions.config.OpTimeout
